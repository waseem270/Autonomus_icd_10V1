...
def _apply_strict_output_filter(unified_results: list, single_pass: bool = False) -> list:
    ...
    for r in unified_results:
        ...
        # EXCEPTION: Z85.x (personal history of malignant neoplasm) codes are
        # kept because they are status codes, not active cancers.
        dname = r.get("disease", r.get("disease_name", ""))
        icd_for_cancer_check = r.get("icd_code", "")
        is_z85_history_code = icd_for_cancer_check.startswith("Z85")
        if _CANCER_PATTERN.search(dname) and not in_visit_assessment and not is_z85_history_code:
